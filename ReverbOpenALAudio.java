 ...
 * 
 */
package org.jevon.onitest.gdx.desktop.audio;
...
import org.lwjgl.demo.openal.EFXUtil;
...
/**
 * Based off
 * https://github.com/abvadabra/libgdx-audio-effects/blob/master/core/src/com/badlogic/gdx/backends/lwjgl/audio/ReverbOpenALAudio.java
 * 
 * @author Jevon
 *
 */
public class ReverbOpenALAudio extends OpenALLwjgl3Audio {
	...
	public static final Reverb REVERB_SMALL = new Reverb(0.15F, 0.0F, 1.0F, 0.2F, 0.99F, 0.6F, 2.5F, 0.001F, 1.26F,
			0.011F, 0.944F, 0.16F);

	public static final Reverb REVERB_MEDIUM = new Reverb(0.55F, 0.0F, 1.0F, 0.3F, 0.99F, 0.7F, 0.2F, 0.015F, 1.26F,
			0.011F, 0.944F, 0.15F);

	public static final Reverb REVERB_BIG = new Reverb(1.68F, 0.1F, 1.0F, 0.5F, 0.99F, 0.7F, 0.0F, 0.021F, 1.26F,
			0.021F, 0.944F, 0.13F);
	...
	public ReverbOpenALAudio(int simultaneousSources, int deviceBufferCount, int deviceBufferSize) {
		...
		/*
		 * if(!EFXUtil.isEfxSupported()) return;
		 */
	...
	public static void createInstance() {
		...
		instance = new ReverbOpenALAudio(audioDeviceSimultaneousSources, audioDeviceBufferCount, audioDeviceBufferSize);
	}

	public static ReverbOpenALAudio overrideAudio(Application app, Lwjgl3ApplicationConfiguration config) {
		// copied from default config
		int audioDeviceSimultaneousSources = 16;
		int audioDeviceBufferSize = 512;
		int audioDeviceBufferCount = 9;

		return new ReverbOpenALAudio(audioDeviceSimultaneousSources, audioDeviceBufferCount, audioDeviceBufferSize);

		/*
		 * //if(!config.disableAudio) { //Destroy original audio and replace with ours
		 * if (app.getAudio() instanceof OpenALAudio) ((OpenALAudio)
		 * app.getAudio()).dispose(); try { audio = new
		 * ReverbOpenALAudio(config.audioDeviceSimultaneousSources,
		 * config.audioDeviceBufferCount, config.audioDeviceBufferSize); } catch
		 * (Throwable t) { app.log("LwjglApplication",
	...
	private void checkAlError() {
		...
		if (error == AL10.AL_INVALID_NAME)
			...
			throw new IllegalStateException("Unknown  AL exception: " + error);
	}
	...
	public static class Reverb {
		/**
		 * Controls reverb decay time. (similiar to echo) Ranges from 0.1 (suitable for
		 * very small room) to 20.0 (large room)
		 */
		public float decayTime;
		/**
		 * Controls the coloration of the late reverb Value range: (0; 1]
		 */
		public float density;
		/**
		 * Controls the echo density when the reverb is decaying Value range: [0; 1]
		 */
		public float diffusion;
		/**
		 * Controls the reflected sound that the reverb adds to all sound sources
		 * attached to. Value range [0; 1] Default: 0.89
		 */
		public float gain;
		/**
		 * Controls the reflected sound at high frequences Value range: [0; 1] Default:
		 * 0.0
		 */
		public float gainHF;
		/**
		 * Adjusts the spectral quality of the [decayTime] Value range: (0; 20.0]
		 * Default: 0.83
		 */
		public float decayHFRatio;
		/**
		 * Overall amount of the initial reflections relative to [gain] Value range: [0,
		 * 3.16] (from -100dB to +10dB) Default: 0.05
		 */
		public float reflectionsGain;
		/**
		 * Delay between the arrival time of the direct path from the source to the
		 * first from the source. Value range: [0; 0.3] (0 to 300 milliseconds)
		 *
		 */
		public float reflectionsDelay;
		/**
		 * Controls the overall amount of LATE (not initial) reverb relative to the
		 * [gain] Value range: [0; 10.0] Default: 1.26
		 */
		public float lateReverbGain;
		/**
		 * Defines the starting time of the late reverb relative to the time of the
		 * initial reflection Value range: [0.0; 0.1] Default: 0.011
		 */
		public float lateReverbDelay;
		/**
		 * Controls the distance-dependent attenuation at high frequencies caused by the
		 * propagation minimum Value range: [0.892; 1.0] Default: 0.994
		 */
		public float airAbsirptionGainHF;
		/**
		 * Attenuates the reflected sound Value range: [0.0; 10.0] Default: 0.0
		 */
